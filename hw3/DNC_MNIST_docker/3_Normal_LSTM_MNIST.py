...
class TestModel_LSTM():
    def __init__(self, name, batch_size = 10, time_step = 28, input_dim = 28, learning_rate = 0.001):
        ...
        self.y = tf.placeholder(dtype=tf.float32, shape=[self.batch_size, self.timesteps, self.input_dim])
    ...
    with tf.Session() as sess:
        ...
        for i in np.arange(epoch_train):        #loop over all batches
            ...
            for j in np.arange(n_batch):
                ...
                cost, _, states, loss_ = sess.run([ simpleLSTM.cost, simpleLSTM.optimizer,simpleLSTM.states, simpleLSTM.loss], feed_dict={simpleLSTM.X:X_train_batch, simpleLSTM.y:X_train_batch})
                # print("loss:",loss_, "   cost:",cost)
                # input("press enter to continue")
                epoch_loss+=cost
                if j%600==0:
                    print('training loss:', epoch_loss/(j+1))
